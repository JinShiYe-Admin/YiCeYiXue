...
function setImg(imgURL,imgFlag) {
	...
	if(imgURL == null || imgURL.length == 0) {
		if (imgFlag==1) {//订购默认图
			tempUrl = '../../img/auto_order_img.png';
		} else{
			tempUrl = '../../img/logo.png';
		}
	} else {
		var myDate = new Date();
...
var postDataEncry = function(url, encryData, commonData, flag, callback) {
	...
	signHmacSHA1.sign(signTemp, 'jsy309', function(sign) {
		//组装发送握手协议需要的data
		//合并对象
		var tempData = mui.extend(encryData, commonData);
		//添加签名
		tempData.sign = sign;
...
var postDataEncry1 = function(encryData, commonData, flag) {
	...
	console.log('合并数组signArr:' + signArr);
	//拼接登录需要的签名
...
var tempPro = function(url, data0, callback) {
	checkNewWork(callback);
	console.log('绑定data:' + JSON.stringify(data0));
	var xhr = new XMLHttpRequest();
	xhr.open("post", url, true);
	xhr.timeout = 10000; //10秒超时
	xhr.contentType = 'application/json;';
